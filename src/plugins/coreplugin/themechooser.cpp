...
QString ThemeEntry::displayName() const
{
    if (m_displayName.isEmpty() && !m_filePath.isEmpty()) {
        ...
        m_displayName = settings.value(QLatin1String(themeNameKey), Tr::tr("unnamed")).toString();
    }
...
public:
    ...
    const ThemeEntry &themeAt(int index) const { return m_themes.at(index); }
    ...
    : d(new ThemeChooserPrivate)
{
    ...
    const int selected = Utils::indexOf(themes,
                                        Utils::equal(&ThemeEntry::id, ThemeEntry::themeSetting()));
    if (selected >= 0)
...
static QString defaultThemeId()
{
    ...
    case Qt::ColorScheme::Light: return "light-2024";
    case Qt::ColorScheme::Dark: return "dark-2024";
    default: return "flat";
    }
}
...
void ThemeChooser::apply()
{
    ...
    if (currentThemeId != themeId) {
        ...
        settings->setValueWithDefault(Core::Constants::SETTINGS_THEME, themeId, defaultThemeId());
...
static void addThemesFromPath(const QString &path, QList<ThemeEntry> *themes)
{
    ...
    for (const QString &fileName : std::as_const(themeList)) {
        ...
        const bool addTheme = ICore::isQtDesignStudio() == id.startsWith("design");
        if (addTheme)
...
QList<ThemeEntry> ThemeEntry::availableThemes()
{
    ...
    static const auto installThemeDir = ICore::resourcePath("themes");
    ...
    const int defaultIndex = Utils::indexOf(themes,
                                            Utils::equal(&ThemeEntry::id,
...
Id ThemeEntry::themeSetting()
{
    const Id setting = Id::fromSetting(
        ICore::settings()->value(Core::Constants::SETTINGS_THEME, defaultThemeId()));
