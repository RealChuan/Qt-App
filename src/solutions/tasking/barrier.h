...
ExecutableItem signalAwaiter(const typename QtPrivate::FunctionPointer<Signal>::Object *sender,
                             Signal signal)
{
...
public:
    ...
    explicit When(const CustomTask<Task, Adapter, Deleter> &customTask,
                  Signal signal,
                  WorkflowPolicy policy = WorkflowPolicy::StopOnError)
...
private:
    template<typename Task, typename Adapter, typename Deleter, typename Signal>
    BarrierKickerGetter kickerForSignal(const CustomTask<Task, Adapter, Deleter> &customTask,
                                        Signal signal)
    {
            ...
            const auto wrappedSetupHandler =
                [originalSetupHandler = std::move(handler.m_taskAdapterSetupHandler),
                 ...
                 signal](void *taskAdapter) {
                    ...
                    QObject::connect(adapter->task.get(),
                                     signal,
