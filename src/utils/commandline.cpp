...
static void envExpandWin(QString &args, const Environment *env, const QString &pwd)
{
    ...
    int off = 0;
next:
    for (int prev = -1, that; (that = args.indexOf(QLatin1Char('%'), off)) >= 0;
         prev = that, off = that + 1) {
        if (prev >= 0) {
            const QString var = args.mid(prev + 1, that - prev - 1).toUpper();
            const QString val = (var == cdName && !pwd.isEmpty()) ? QDir::toNativeSeparators(pwd)
                                                                  : env->expandedValueForKey(var);
...
static QString prepareArgsWin(const QString &_args,
...
static QStringList splitArgsWin(const QString &_args,
                                bool abortOnMeta,
                                ProcessArgs::SplitError *err,
                                const Environment *env,
                                const QString &pwd)
...
static QStringList splitArgsUnix(const QString &args,
                                 bool abortOnMeta,
                                 ProcessArgs::SplitError *err,
                                 const Environment *env,
                                 const QString &pwd)
{
    ...
    for (int pos = 0;;) {
        ...
        bool hadWord = false;
        if (c == QLatin1Char('~')) {
            if (pos >= args.length() || args.unicode()[pos].isSpace()
                ...
                nextq:
                    ...
                    if (c == QLatin1Char('\\')) {
                        ...
                        if (c != QLatin1Char('"') && c != QLatin1Char('\\')
                            && !(abortOnMeta && (c == QLatin1Char('$') || c == QLatin1Char('`'))))
                            cret += QLatin1Char('\\');
...
QStringList ProcessArgs::splitArgs(const QString &args,
                                   OsType osType,
                                   bool abortOnMeta,
                                   ProcessArgs::SplitError *err,
                                   const Environment *env,
...
QString ProcessArgs::prepareShellArgs(const QString &args,
                                      SplitError *err,
                                      OsType osType,
                                      const Environment *env,
                                      const FilePath &pwd)
...
bool ProcessArgs::prepareCommand(const CommandLine &cmdLine,
                                 QString *outCmd,
                                 QString *outArgs,
                                 const Environment *env,
                                 const FilePath &pwd)
    ...
    if (err == ProcessArgs::SplitOk) {
        *outCmd = executable.path();
        ...
        if (osType == OsTypeWindows) {
            ...
            *outArgs = "/v:off /s /c \"" + quoteArgWin(executable.nativePath()) + ' ' + arguments
                       + '"';
        } else {
...
bool ProcessArgs::ArgIterator::next()
{
    ...
    if (m_osType == OsTypeWindows) {
        ...
        enum {
            ...
            = NoVar;      // inside a potential env variable expansion
        ...
        for (;; m_pos++) {
            ...
            if (shellState == ShellBasic && cc == '^') {
                varState = NoVar;
                shellState = ShellEscaped;
            } else if ((shellState == ShellBasic && isMetaCharWin(cc))
                       || !cc) { // A "bit" simplistic ...
