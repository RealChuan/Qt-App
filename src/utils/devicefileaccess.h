...
protected:
    ...
    virtual Result<> iterateDirectory(const FilePath &filePath,
...
protected:
    ...
    Result<std::optional<FilePath>> refersToExecutableFile(
        const FilePath &filePath, FilePath::MatchScope matchScope) const override;

    Result<> iterateDirectory(const FilePath &filePath,
                              const FilePath::IterateDirCallback &callBack,
...
protected:
    virtual Result<RunResult> runInShellImpl(const CommandLine &cmdLine,
                                             const QByteArray &inputData = {}) const
        = 0;
    Result<QByteArray> runInShell(const CommandLine &cmdLine,
                                  const QByteArray &stdInData = {}) const;
    Result<bool> runInShellSuccess(const CommandLine &cmdLine,
                                   const QByteArray &stdInData = {}) const;
