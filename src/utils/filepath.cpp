...
FilePath FilePath::fromParts(const QStringView scheme,
...
bool FilePath::equals(const FilePath &first, const FilePath &second, Qt::CaseSensitivity cs)
{
    ...
    return first.pathView().compare(second.pathView(), cs) == 0 && first.host() == second.host()
...
static FilePaths appendExeExtensions(const FilePath &executable, FilePath::MatchScope matchScope)
{
    ...
    if (executable.osType() == OsTypeWindows && suffix.isEmpty()) {
        switch (matchScope) {
        case FilePath::ExactMatchOnly: break;
        case FilePath::WithExeSuffix: result.append(executable.stringAppended(".exe")); break;
        case FilePath::WithBatSuffix: result.append(executable.stringAppended(".bat")); break;
...
QString FilePath::displayName(const QString &args) const
{
    ...
    return Tr::tr("%1 %2 on %3", "File and args on device").arg(fullPath, args, deviceName);
}
...
void FilePath::searchHereAndInParents(
...
Result<> FilePath::copyFile(const FilePath &target) const
{
    if (!isSameDevice(target)) {
        ...
        if (!contents) {
            return ResultError(Tr::tr("Error while trying to copy file: %1").arg(contents.error()));
        }
...
Result<> FilePath::createSymLink(const FilePath &symLink) const
{
    if (!isSameDevice(symLink)) {
        return ResultError(
            Tr::tr("Cannot create symbolic link to \"%1\" at \"%2\": Paths do not refer to the "
                   "same device.")
                .arg(toUserOutput(), symLink.toUserOutput()));
    }
...
Result<> FilePath::renameFile(const FilePath &target) const
{
    ...
    return ResultError(Tr::tr("Failed to move %1 to %2. Removing the source file failed: %3")
                           .arg(toUserOutput())
...
FilePath FilePath::canonicalPath() const
{
    ...
    const HANDLE fileHandle = CreateFile(toUserOutput().toStdWString().c_str(),
                                         GENERIC_READ,
                                         FILE_SHARE_READ,
                                         nullptr,
                                         OPEN_EXISTING,
    ...
    if (fileHandle != INVALID_HANDLE_VALUE) {
        ...
        const auto length = GetFinalPathNameByHandleW(fileHandle,
                                                      normalizedPath,
                                                      MAX_PATH,
...
FilePath FilePaths::commonPath() const
{
    ...
    auto sameScheme = [&commonScheme](const FilePath &fp) { return commonScheme == fp.scheme(); };
    ...
    auto sameHost = [&commonHost](const FilePath &fp) { return commonHost == fp.host(); };
