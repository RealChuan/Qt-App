        ...
        for (int i = 0; i < itemList.size(); ++i) {
            ...
            if (spaceX < 0) {
                if (i < itemList.size() - 1) {
                    spaceX = wid->style()->combinedLayoutSpacing(item->controlTypes(),
                                                                 itemList.at(i + 1)->controlTypes(),
                                                                 Qt::Horizontal);
                }
                if (spaceX < 0)
                    spaceX = 0;
            ...
            [this, &geometries, effectiveRect](int count, int beforeIndex, int lineHeight) {
                ...
                for (int j = 0; j < count; ++j) {
                    ...
                    currentItem->setGeometry(
                        QRect(QPoint(geom.x() + xd, geom.y() + (lineHeight - geom.height()) / 2),
                              geom.size()));
                }
            };
        int indexInRow = 0;
    ...
    if (QGridLayout *lt = asGrid()) {
        ...
        for (const LayoutItem &item : std::as_const(pendingItems)) {
            ...
            if (item.widget) {
                ...
                lt->addWidget(createLabel(item.text),
                              currentGridRow,
                              currentGridColumn,
                              item.spanRows,
                              item.spanCols,
