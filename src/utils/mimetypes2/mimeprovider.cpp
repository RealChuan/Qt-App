...
void MimeBinaryProvider::addFileNameMatches(const QString &fileName, MimeGlobMatchResult &result)
{
    ...
    numMatches = matchGlobList(result,
                               m_cacheFile.get(),
    ...
    if (numMatches == 0) {
        ...
        if (matchSuffixTree(result,
                            ...
                            false)) {
            ++numMatches;
        } else if (matchSuffixTree(result,
                                   m_cacheFile.get(),
                                   numRoots,
                                   firstRootOffset,
                                   fileName,
...
int MimeBinaryProvider::matchGlobList(MimeGlobMatchResult &result,
                                      CacheFile *cacheFile,
                                      int off,
            ...
            if (!success) {
                for (int i = 0; i < numChildren; ++i) {
                    ...
                    if (caseSensitiveCheck || !caseSensitive) {
                        result.addMatch(mimeType,
                                        weight,
                                        QString('*') + QStringView{fileName}.mid(charPos + 1),
...
MimeBinaryProvider::MimeTypeExtraMap::const_iterator MimeBinaryProvider::loadMimeTypeExtra(
            ...
            while (xml.readNextStartElement()) {
                ...
                if (tag == "comment"_L1) {
                    ...
                    continue; // we called readElementText, so we're at the EndElement already.
                           ...
                           == "glob-deleteall"_L1) { // as written out by shared-mime-info >= 0.70
...
QLatin1StringView MimeBinaryProvider::iconForMime(CacheFile *cacheFile,
                                                  int posListOffset,
