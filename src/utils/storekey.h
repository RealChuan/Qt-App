...
public:
    ...
    Key(const QByteArray &key)
        : data(key)
    {}
    Key(const QByteArrayView key)
        : data(key.toByteArray())
    {}

    template<int N>
    Key(const char (&key)[N])
        : data(key)
    {}
    ...
    friend Key operator+(const Key &a, char b) { return Key(a.data + b); }
...
UTILS_EXPORT Key keyFromString(const QString &str);
UTILS_EXPORT QString stringFromKey(const Key &key);
