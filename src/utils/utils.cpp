...
void Utils::setQSS(const QStringList &qssFilePaths)
{
    ...
    for (const auto &path : qssFilePaths) {
        QFile file(path);
        if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            qWarning() << QString("Cannot open the file: %1").arg(path);
            continue;
        }
...
auto Utils::fileSize(const QFileInfo &localPath) -> qint64
{
    if (localPath.isDir()) {
        return calculateDir(localPath.filePath());
    }
