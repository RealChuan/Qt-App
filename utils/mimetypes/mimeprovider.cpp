            ...
            if (!success) {
                for (int i = 0; i < numChildren; ++i) {
                    ...
                    if (caseSensitiveCheck || !caseSensitive) {
                        QString pattern;
                        pattern += QLatin1Char('*');
                        pattern += QStringView{fileName}.mid(charPos + 1);
                        result.addMatch(QLatin1String(mimeType),
                                        weight,
                                        pattern,
...
void MimeBinaryProvider::addAllMimeTypes(QList<MimeType> &result)
{
    ...
    if (result.isEmpty()) {
        ...
        for (const QString &name : std::as_const(m_mimetypeNames))
            if (std::find_if(result.constBegin(),
                             result.constEnd(),
                             [name](const MimeType &mime) -> bool { return mime.name() == name; })
                == result.constEnd())
                result.append(mimeTypeForNameUnchecked(name));
    }
}
...
auto MimeBinaryProvider::loadMimeTypePrivate(MimeTypePrivate &data) -> bool
{
    ...
    if (it == m_mimetypeExtra.constEnd()) {
        ...
        QString mimeFile = m_directory + QLatin1Char('/') + data.name.toLower()
                           + QLatin1String(".xml");
        if (!QFile::exists(mimeFile))
            mimeFile = m_directory + QLatin1Char('/') + data.name
                       + QLatin1String(".xml"); // pre-1.3
        ...
        if (xml.readNextStartElement()) {
            ...
            if (name.compare(data.name, Qt::CaseInsensitive))
                qWarning() << "Got name" << name << "in file" << mimeFile << "expected"
                           << data.name;
            ...
            while (xml.readNextStartElement()) {
                ...
                if (tag == QLatin1String("comment")) {
                    ...
                    if (lang.isEmpty()) {
                        lang = QLatin1String(
                            "default"); // no locale attribute provided, treat it as default.
                    }
                    extra.localeComments.insert(lang, text);
                    continue; // we called readElementText, so we're at the EndElement already.
                } else if (tag
                           == QLatin1String(
                               "glob-deleteall")) { // as written out by shared-mime-info >= 0.70
                    extra.globPatterns.clear();
                    mainPattern.clear();
                } else if (tag
                           == QLatin1String("glob")) { // as written out by shared-mime-info >= 0.70
                    const QString pattern
                        = xml.attributes().value(QLatin1String("pattern")).toString();
...
void MimeXMLProvider::addAllMimeTypes(QList<MimeType> &result)
{
    if (result.isEmpty()) { // fast path
        ...
        for (auto it = m_nameMimeTypeMap.constBegin(), end = m_nameMimeTypeMap.constEnd();
            ...
            if (std::find_if(result.constBegin(),
                             result.constEnd(),
                             [newMime](const MimeType &mime) -> bool {
